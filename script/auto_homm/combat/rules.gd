...
@export var enemies_query: teCombatTargetQueryBase
@export var attack_target_priority: Dictionary[teCombatAttack.Targeting, teCombatTargetUnitPriorityBase] = {}
...
func auto_command(runtime: teCombatRuntime, state: teCombatState) -> teCombatCommandBase:
	...
	if active_unit.can_cast():
		var skill_target := teCombatTargeting.auto(
			state,
			active_unit_id,
			active_unit.skill.targeting_profile
		)
		if teCombatTargeting.is_valid(skill_target):
			return teCombatCommands.cast_skill(active_unit_id, skill_target)
	var attack_target := teCombatTargeting.prioritized(
		state,
		active_unit_id,
		enemies_query,
		attack_target_priority[active_unit.attack_targeting]
	)
	if not teCombatTargeting.is_valid(attack_target):
		return teCombatCommands.wait()
	var unit_targget := attack_target as teCombatTargetUnit
	if unit_targget == null:
		return teCombatCommands.wait()
	return teCombatCommands.attack(active_unit_id, unit_targget.single())
...
func expand(
	...
	if command is teCombatCommandWait:
		expanded.append(teCombatActions.initiative_advance())
	if command is teCombatCommandAttack:
		...
		if not unit.in_attack_range(target):
			...
			expanded.append(teCombatActions.move(command.unit_id, movement_path))
			if movement_path.length() > unit.stats.movement_range:
				movement_path.limit(unit.stats.movement_range)
				expanded.append(teCombatActions.initiative_advance())
				return expanded
		expanded.append(teCombatActions.attack(command.unit_id, command.target_id))
		expanded.append(teCombatActions.initiative_advance())
	if command is teCombatCommandCastSkill:
		expanded.append(teCombatActions.cast(command.unit_id, command.target))
		var unit := state.unit(command.unit_id)
...
func is_valid(command: teCombatCommandBase) -> bool:
	return true
