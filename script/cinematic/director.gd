class_name CinematicDirector extends CinematicDirectorBase
...
func direct_take(action: teVisualActionBase, speed_scale := 1.0) -> CinematicTake:
	if action is teVisualActionUnitAct:
		...
		if visuals == null:
			return CinematicTakes.fail("Can't get unit visuals to act")
		var actual_act := visuals.actual_act(action.act)
		if visuals.knows_act(actual_act):
			visuals.play_act(actual_act, speed_scale)
			return CinematicTakes.instant()
		if action.can_be_unknown:
			return CinematicTakes.instant()
		else:
			return CinematicTakes.fail("Visuals don't know act")
	if action is teVisualActionUnitWindup:
		var visuals := board.get_unit_visuals(action.unit_id)
		if visuals == null:
			return CinematicTakes.fail("Can't get unit visuals to windup")
		if not visuals.knows_windup(action.act):
			return CinematicTakes.fail("Unit visuals don't know windup")
		return CinematicTakes.async(func(): await visuals.play_windup(action.act, speed_scale))
	if action is teVisualActionUnitCombo:
		var visuals = board.get_unit_visuals(action.unit_id)
		if visuals == null:
			return CinematicTakes.fail("Can't get unit visuals to make combo")
		if not visuals.knows_combo(action.base_act, action.total):
			return CinematicTakes.fail("Visuals don't know combo")
		return CinematicTakes.async(func():
	...
	if action is teVisualActionUnitFlash:
		...
		if unit == null:
			return CinematicTakes.fail("Can't get unit to flash")
		var duration = action.time / speed_scale
		unit.flash(duration)
		if action.wait:
			return CinematicTakes.timer(self, duration)
		else:
			return CinematicTakes.instant()
	...
	if action is teVisualActionUnitMove:
		var unit := board.get_unit(action.unit_id)
		if unit == null:
			return CinematicTakes.fail("Can't get unit to move")
		var visuals := board.get_unit_visuals(action.unit_id)
	...
	if action is teVisualActionWait:
		return CinematicTakes.timer(self, action.time_sec / speed_scale)
	return CinematicTakes.fail("Unknown action type to direct")
